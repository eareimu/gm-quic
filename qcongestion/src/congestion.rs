...
impl RcvdRecords {
    ...
    fn ack(&mut self, ack: u64) -> Option<u64> {
        let largest = self.ack_sent.get(&ack)?;
        const THRESHOLD: u64 = 3;
        let rotate = largest.saturating_sub(THRESHOLD);
        self.rcvd_queue.retain(|&(pn, _)| pn > rotate);
        self.ack_sent.remove(&ack);
        Some(rotate)
    }
}
